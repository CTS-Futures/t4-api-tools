...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
       ...
       /*  connectionPane.setOnConnect(() -> {
                ...
                client.connect(() -> Platform.runLater(() -> {
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            System.out.println("Subscribing to " + marketId + " [source: default/user]");
                            Platform.runLater(() ->
        ...
        connectionPane.setOnConnect(() -> {
            try {
                client.connect(() -> {
            // After connection succeeds, update UI
        ...
        new Thread(() -> {
            try {
                ...
                if (ex.getMessage().contains("404")) {
                    System.err.println("Market not found, displaying as unavailable.");
                    Platform.runLater(() -> {
                        marketPane.updateSymbol(contract.toString());  // ⬅️ keep the symbol
    ...
    dialog.show();  // Keeps your existing Swing dialog intact
});
